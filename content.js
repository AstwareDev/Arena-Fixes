...
const LMARENA_STYLE_ID = 'arena-fixes-lmarena-theme';
const LMARENA_LOGO_ID = 'arena-fixes-lmarena-logo';
const AUTO_SCROLL_STYLE_ID = 'arena-fixes-auto-scroll-disable';
const PROFILE_PIC_ATTR = 'data-arena-fixes-pfp';
...
function hideOriginalButton(btn) {
  btn.setAttribute('data-arena-fixes-original-copy-hidden', '1');
  btn.style.opacity = '0';
...
function unhideOriginalButton(btn) {
  ...
  btn.style.minWidth = '';
  btn.style.minHeight = '';
  btn.removeAttribute('data-arena-fixes-original-copy-hidden');
}

function ensureStyleTag() {
  if (document.getElementById('arena-fixes-copy-style')) return;
  const style = document.createElement('style');
  style.id = 'arena-fixes-copy-style';
  style.textContent = `
    ...
    .arena-fixes-copy-host:hover .arena-fixes-copy-btn {
      opacity: 1;
    }
    ...
    .arena-fixes-copy-btn.arena-fixes-copied {
      opacity: 1 !important;
    }
    .arena-fixes-copy-btn.arena-fixes-copied .arena-fixes-copy-inner {
      opacity: 0;
    }
    .arena-fixes-copy-btn.arena-fixes-copied .arena-fixes-copy-check {
      opacity: 1;
    }
  `;
...
function addBottomButtonToRow(row) {
  ...
  if (bubbleWrap.querySelector('[data-arena-fixes-bottom-copy-wrap]')) {
  ...
  wrap.setAttribute('data-arena-fixes-bottom-copy-wrap', '1');
  ...
  const bottomBtn = buildBottomButton(originalCopy);
...
function disableBottomCopyButtons() {
  document.querySelectorAll('[data-arena-fixes-bottom-copy-wrap]').forEach(el => el.remove());
  document.querySelectorAll('[data-arena-fixes-original-copy-hidden="1"]').forEach(unhideOriginalButton);
}

function getBottomMostUserMessageText() {
  const rows = getUserMessageRows();
  const bottomRow = getVisualBottomMostRow(rows);
  if (!bottomRow) return '';
  return getMessageText(bottomRow);
}
...
async function applyRawMarkdownToRow(row) {
  if (row.getAttribute('data-arena-fixes-raw-applied') === '1') return;
  const bubble = getUserBubble(row);
  if (!bubble) return;
  const prose = bubble.querySelector('.prose');
  if (!prose) return;
  ...
  if (!rawText) return;
  row.setAttribute('data-arena-fixes-raw-applied', '1');
  const pre = document.createElement('pre');
  pre.setAttribute('data-arena-fixes-raw-pre', '1');
...
function disableRawMarkdown() {
  ...
  if (rawMarkdownObserver) {
    rawMarkdownObserver.disconnect();
    rawMarkdownObserver = null;
  }
  document.querySelectorAll('[data-arena-fixes-raw-applied="1"]').forEach(row => {
    row.removeAttribute('data-arena-fixes-raw-applied');
    const bubble = getUserBubble(row);
    if (!bubble) return;
    const prose = bubble.querySelector('.prose');
    if (prose) prose.style.display = '';
    bubble.querySelectorAll('[data-arena-fixes-raw-pre="1"]').forEach(pre => pre.remove());
  });
}
...
function applyLMArenaSidebarRounding() {
  ...
  if (!sidebar) return;
  if (sidebar.getAttribute('data-arena-fixes-rounded') === '1') return;
  ...
  sidebar.setAttribute('data-arena-fixes-rounded', '1');
...
function removeLMArenaSidebarRounding() {
  ...
  sidebar.removeAttribute('data-arena-fixes-rounded');
...
function applyLMArenaInputBar() {
  const addFilesBtn = [...document.querySelectorAll('button[aria-label="Add files and more"]')].find(btn => btn.querySelector('.lucide-paperclip'));
  if (!addFilesBtn) return;
  if (addFilesBtn.getAttribute('data-arena-fixes-input-bar') === '1') return;
  ...
  addFilesBtn.setAttribute('data-arena-fixes-input-bar', '1');
}

function removeLMArenaInputBar() {
  const addFilesBtn = document.querySelector('button[data-arena-fixes-input-bar="1"]');
  if (!addFilesBtn) return;
  addFilesBtn.removeAttribute('data-arena-fixes-input-bar');

  const svg = addFilesBtn.querySelector('svg');
  if (svg) {
    svg.innerHTML = '<path d="M13.234 20.252 21 12.3"></path><path d="m16 6-8.414 8.586a2 2 0 0 0 0 2.828 2 2 0 0 0 2.828 0l8.414-8.586a4 4 0 0 0 0-5.656 4 4 0 0 0-5.656 0l-8.415 8.585a6 6 0 1 0 8.486 8.486"></path>';
  }
}

function applyLMArenaIcons() {
  const allSvgs = [...document.querySelectorAll('svg.h-5.w-5')];

  for (const svg of allSvgs) {
    if (svg.getAttribute('data-arena-fixes-icon')) continue;
    ...
    if (hasNewChat) {
      svg.setAttribute('data-arena-fixes-icon', 'newchat');
      svg.setAttribute('width', '1.8em');
      svg.setAttribute('height', '1.8em');
    ...
    if (hasLeaderboard) {
      svg.setAttribute('data-arena-fixes-icon', 'leaderboard');
      svg.setAttribute('width', '1.8em');
      svg.setAttribute('height', '1.8em');
  ...
  const allSvgs16 = [...document.querySelectorAll('svg[width="16"][height="16"]')];
  for (const svg of allSvgs16) {
    if (svg.getAttribute('data-arena-fixes-icon') === 'chain-replacement') continue;
    if (svg.getAttribute('data-arena-fixes-icon') === 'chain') continue;
    const paths = svg.querySelectorAll('path');
    const hasChain = [...paths].some(p => p.getAttribute('d')?.includes('M22 14V6'));
    if (!hasChain) continue;

    svg.setAttribute('data-arena-fixes-icon', 'chain');
    ...
    newSvg.setAttribute('data-arena-fixes-icon', 'chain-replacement');
...
function applyLMArenaModalityIcons() {
  const buttons = [...document.querySelectorAll('button[data-modality-button="true"]')];
  for (const btn of buttons) {
    if (btn.getAttribute('data-arena-fixes-modality-icon')) continue;
    const label = btn.getAttribute('aria-label');

    if (label === 'Code') {
      btn.setAttribute('data-arena-fixes-modality-icon', 'code');
    ...
    if (label === 'Search') {
      btn.setAttribute('data-arena-fixes-modality-icon', 'search');
      const svg = btn.querySelector('svg');
      if (svg) {
        svg.setAttribute('viewBox', '0 0 24 24');
  ...
  for (const btn of submitBtns) {
    ...
    if (!svg) continue;
    const paths = svg.querySelectorAll('path');
    const hasArrow = [...paths].some(p => p.getAttribute('d')?.includes('M3 12L21 12'));
    if (!hasArrow) continue;
    btn.setAttribute('data-arena-fixes-modality-icon', 'submit');
    svg.setAttribute('viewBox', '0 0 24 24');
...
function removeLMArenaModalityIcons() {
  document.querySelectorAll('[data-arena-fixes-modality-icon="code"]').forEach(btn => {
    btn.removeAttribute('data-arena-fixes-modality-icon');
    const svg = btn.querySelector('svg');
  ...
  document.querySelectorAll('[data-arena-fixes-modality-icon="search"]').forEach(btn => {
    btn.removeAttribute('data-arena-fixes-modality-icon');
    const svg = btn.querySelector('svg');
  ...
  document.querySelectorAll('[data-arena-fixes-modality-icon="submit"]').forEach(btn => {
    btn.removeAttribute('data-arena-fixes-modality-icon');
    const svg = btn.querySelector('svg');
...
function hideYouTubeCTA() {
  document.querySelectorAll('video').forEach(video => {
    if (video.src && video.src.includes('youtube-channel-ad')) {
      const cta = video.closest('.animate-in, [style*="will-change"]');
      if (cta) {
        cta.setAttribute('data-arena-fixes-hidden-cta', '1');
...
function showYouTubeCTA() {
  document.querySelectorAll('[data-arena-fixes-hidden-cta="1"]').forEach(el => {
    el.style.removeProperty('display');
    el.removeAttribute('data-arena-fixes-hidden-cta');
  });
}

function removeLMArenaIcons() {
  const newChat = document.querySelector('svg[data-arena-fixes-icon="newchat"]');
  if (newChat) {
    newChat.removeAttribute('data-arena-fixes-icon');
    newChat.setAttribute('width', '1.5em');
    newChat.setAttribute('height', '1.5em');
  ...
  const leaderboard = document.querySelector('svg[data-arena-fixes-icon="leaderboard"]');
  if (leaderboard) {
    leaderboard.removeAttribute('data-arena-fixes-icon');
    leaderboard.setAttribute('width', '1.5em');
    leaderboard.setAttribute('height', '1.5em');
  ...
  document.querySelectorAll('svg[data-arena-fixes-icon="chain"]').forEach(svg => {
    svg.removeAttribute('data-arena-fixes-icon');
    svg.style.removeProperty('display');
  });
  document.querySelectorAll('svg[data-arena-fixes-icon="chain-replacement"]').forEach(svg => svg.remove());

  showYouTubeCTA();
}

function applyLMArenaFavicon() {
  if (document.getElementById('arena-fixes-favicon')) return;
  const link = document.createElement('link');
  link.id = 'arena-fixes-favicon';
  link.rel = 'icon';
...
function removeLMArenaFavicon() {
  const link = document.getElementById('arena-fixes-favicon');
  if (link) link.remove();
  const existing = document.querySelector('link[rel="icon"], link[rel="shortcut icon"]');
...
function applyLMArenaSidebarIcon() {
  ...
  for (const svg of allSvgs) {
    if (svg.getAttribute('data-arena-fixes-icon') === 'sidebar') continue;
    const paths = svg.querySelectorAll('path');
    const hasSidebar = [...paths].some(p => p.getAttribute('d')?.includes('M9.5 21V3'));
    if (!hasSidebar) continue;

    svg.setAttribute('data-arena-fixes-icon', 'sidebar');
    ...
    img.setAttribute('data-arena-fixes-sidebar-img', '1');
...
function removeLMArenaSidebarIcon() {
  document.querySelectorAll('img[data-arena-fixes-sidebar-img="1"]').forEach(img => img.remove());
  document.querySelectorAll('svg[data-arena-fixes-icon="sidebar"]').forEach(svg => {
    svg.removeAttribute('data-arena-fixes-icon');
    svg.style.display = '';
  });
}
...
function applyLMArenaTheme() {
  if (document.getElementById(LMARENA_STYLE_ID)) return;
  const style = document.createElement('style');
  style.id = LMARENA_STYLE_ID;
  style.textContent = `
    ...
    img[data-arena-fixes-sidebar-img],
    #arena-fixes-lmarena-logo img {
      filter: none;
    }
  `;
  document.head.appendChild(style);
  if (!document.getElementById('arena-fixes-cinzel-font')) {
    const link = document.createElement('link');
    link.id = 'arena-fixes-cinzel-font';
    link.rel = 'stylesheet';
...
function removeLMArenaTheme() {
  stopLogoObserver();
  stopColorWatcher();
  const style = document.getElementById(LMARENA_STYLE_ID);
  ...
  const themeBtn = document.getElementById('arena-fixes-theme-btn');
...
function injectLMArenaLogo() {
  if (document.getElementById(LMARENA_LOGO_ID)) return;
  const svg = findLogoSVG();
  if (!svg) return;
  const container = svg.parentElement;
  if (!container) return;
  svg.style.display = 'none';
  svg.setAttribute('data-arena-fixes-hidden', '1');

  const logo = document.createElement('span');
  logo.id = LMARENA_LOGO_ID;
  logo.setAttribute('data-arena-fixes-logo', '1');
...
function removeLMArenaLogo() {
  const logo = document.getElementById(LMARENA_LOGO_ID);
  if (logo) logo.remove();
  const svg = document.querySelector('svg[data-arena-fixes-hidden="1"]');
  if (svg) {
    svg.style.display = '';
    svg.removeAttribute('data-arena-fixes-hidden');
  }
}
...
function startLogoObserver() {
  ...
  logoObserver = new MutationObserver(() => {
    ...
    logoObserverDebounce = setTimeout(() => {
      ...
      if (!document.getElementById(LMARENA_LOGO_ID)) {
        const svg = findLogoSVG();
        if (svg && !svg.getAttribute('data-arena-fixes-hidden')) {
          svg.style.display = 'none';
          svg.setAttribute('data-arena-fixes-hidden', '1');
        }
        injectLMArenaLogo();
...
async function clickArenaThemeButton(theme) {
  return new Promise(resolve => {
    ...
    const observer = new MutationObserver(() => {
      const btn = [...document.querySelectorAll('button[role="tab"]')].find(b =>
        b.textContent.trim().toLowerCase() === theme
      );
      if (btn) {
        observer.disconnect();
        btn.click();
...
function injectThemeToggleButton() {
  ...
  themeBtn.addEventListener('mouseenter', () => {
    ...
    themeBtn.style.background = 'rgba(128,128,128,0.15)';
  });
...
function startColorWatcher() {
  ...
  colorWatcher = new MutationObserver(() => {
...
function applyLightModeOverride() {
  if (document.getElementById('arena-fixes-light-override')) return;
  const style = document.createElement('style');
  style.id = 'arena-fixes-light-override';
  style.textContent = `
    html, body {
    ...
    html, html.dark, :root, :root.dark {
      ...
      --sidebar-background: 0 0% 95% !important;
      --card: 0 0% 98% !important;
      --popover: 0 0% 100% !important;
      --surface-primary: 0 0% 100% !important;
      --surface-secondary: 0 0% 95% !important;
      --surface-raised: 0 0% 93% !important;
    ...
    * { color: #1a1a1a !important; }
    ...
    [data-sidebar="menu-button"] div.bg-surface-primary {
      background-color: #e8e8e8 !important;
    }
    [data-sidebar="menu-button"][data-active="true"] div.bg-surface-primary {
      background-color: #d8d8d8 !important;
    }
    img[data-arena-fixes-sidebar-img="1"],
    #arena-fixes-lmarena-logo img {
...
function removeLightModeOverride() {
  const style = document.getElementById('arena-fixes-light-override');
  if (style) style.remove();
}

function disableAutoScroll() {
  if (document.getElementById(AUTO_SCROLL_STYLE_ID)) return;
  const style = document.createElement('style');
  style.id = AUTO_SCROLL_STYLE_ID;
  style.textContent = '* { scroll-behavior: auto !important; }';
  document.head.appendChild(style);
  const scrollContainers = document.querySelectorAll('[style*="scroll"]');
  scrollContainers.forEach(el => {
    el.setAttribute('data-arena-fixes-scroll-disabled', '1');
    const originalScroll = el.scrollTop;
    const obs = new MutationObserver(() => {
      if (el.scrollTop !== originalScroll) el.scrollTop = originalScroll;
    });
...
function enableAutoScroll() {
  const style = document.getElementById(AUTO_SCROLL_STYLE_ID);
  if (style) style.remove();
  document.querySelectorAll('[data-arena-fixes-scroll-disabled]').forEach(el => {
    el.removeAttribute('data-arena-fixes-scroll-disabled');
  });
}
...
function isArenaPlaceholderAvatar(src) {
  ...
  return path.includes('/images/avatars/');
}

function getEffectiveProfilePicUrl() {
  if (profilePicUrl && profilePicUrl.startsWith('http')) return profilePicUrl;
...
function findProfilePicFromPage() {
  const selectors = [
    ...
    'img[src*="avatars.githubusercontent"]',
  ...
  try {
    for (let i = 0; i < localStorage.length; i++) {
      ...
      try {
        const pic = deepFindPic(JSON.parse(val), 3);
        if (pic) return pic;
      } catch {}
    }
  } catch {}
...
function deepFindPic(obj, depth) {
  ...
  if (obj.user && typeof obj.user === 'object') {
    const r = deepFindPic(obj.user, depth - 1);
    if (r) return r;
  }
  if (obj.profile && typeof obj.profile === 'object') {
    const r = deepFindPic(obj.profile, depth - 1);
    if (r) return r;
  }
  return null;
}
...
function replaceArenaAvatars() {
  ...
  document.querySelectorAll('span.relative.flex.overflow-hidden.rounded-full').forEach(span => {
    if (span.getAttribute(PROFILE_PIC_ATTR) === '1') return;
    const img = span.querySelector('img.aspect-square');
    if (!img) return;
    if (isArenaPlaceholderAvatar(img.src)) {
      img.setAttribute('data-arena-fixes-original-src', img.src);
      img.src = url;
      img.style.objectFit = 'cover';
      span.setAttribute(PROFILE_PIC_ATTR, '1');
    }
  });

  document.querySelectorAll('img').forEach(img => {
    if (img.getAttribute(PROFILE_PIC_ATTR) === '1') return;
    if (!isArenaPlaceholderAvatar(img.src)) return;
    const parent = img.closest('.rounded-full') ||
                   img.closest('[class*="avatar"]') ||
                   img.closest('[class*="profile"]');
    if (parent || img.classList.contains('rounded-full')) {
      img.setAttribute('data-arena-fixes-original-src', img.src);
      img.setAttribute(PROFILE_PIC_ATTR, '1');
      img.src = url;
      img.style.objectFit = 'cover';
    }
  });
...
async function enableProfilePicFix() {
  ...
  if (!profilePicUrl || !profilePicUrl.startsWith('http')) {
    if (capturedGooglePic) {
      ...
      const data = await new Promise(resolve => chrome.storage.local.get(['capturedGooglePic'], resolve));
      if (data.capturedGooglePic) {
        profilePicUrl = data.capturedGooglePic;
        capturedGooglePic = data.capturedGooglePic;
      }
    }
  }
  ...
  if (!profilePicObserver) {
    ...
    profilePicObserver = new MutationObserver(() => {
      ...
      pfpDebounce = setTimeout(() => {
        pfpDebounce = null;
        if (!profilePicUrl || !profilePicUrl.startsWith('http')) {
          if (capturedGooglePic) profilePicUrl = capturedGooglePic;
        }
        replaceArenaAvatars();
      }, 200);
    });
    profilePicObserver.observe(document.body, {
      childList: true,
      subtree: true,
      attributes: true,
      attributeFilter: ['src']
...
function disableProfilePicFix() {
  ...
  if (profilePicObserver) {
    profilePicObserver.disconnect();
    profilePicObserver = null;
  }
  if (profilePicInterval) {
  ...
  document.querySelectorAll(`[${PROFILE_PIC_ATTR}="1"]`).forEach(el => {
    if (el.tagName === 'IMG') {
      const originalSrc = el.getAttribute('data-arena-fixes-original-src');
      if (originalSrc) {
        el.src = originalSrc;
        el.removeAttribute('data-arena-fixes-original-src');
      }
      el.removeAttribute(PROFILE_PIC_ATTR);
      el.style.objectFit = '';
    } else {
      el.removeAttribute(PROFILE_PIC_ATTR);
      const img = el.querySelector(`img[${PROFILE_PIC_ATTR}]`);
      if (img) {
        const originalSrc = img.getAttribute('data-arena-fixes-original-src');
        if (originalSrc) {
          img.src = originalSrc;
          img.removeAttribute('data-arena-fixes-original-src');
        }
        img.removeAttribute(PROFILE_PIC_ATTR);
        img.style.objectFit = '';
      }
    }
  });
...
function startGooglePicCapture() {
  ...
  const checkImg = (img) => {
    ...
    if (
      ...
      img.src.includes('lh3.google') ||
      img.src.includes('lh4.google') ||
      img.src.includes('lh5.google') ||
      img.src.includes('lh6.google')
    ) {
      const url = img.src;
      if (capturedGooglePic === url) return;
      capturedGooglePic = url;
      chrome.storage.local.set({ capturedGooglePic: url, profilePicUrl: url });
      if (!profilePicUrl || !profilePicUrl.startsWith('http')) {
        profilePicUrl = url;
      }
  ...
  googlePicObserver.observe(document.body, {
    childList: true,
    subtree: true,
    attributes: true,
    attributeFilter: ['src']
...
chrome.storage.local.get(["bottomCopyEnabled", "oldThemeEnabled", "autoScrollDisabled", "profilePicEnabled", "profilePicUrl", "rawMarkdownEnabled"], data => {
  enabledState = !!data.bottomCopyEnabled;
  oldThemeEnabled = !!data.oldThemeEnabled;
  autoScrollDisabled = !!data.autoScrollDisabled;
  profilePicEnabled = !!data.profilePicEnabled;
  profilePicUrl = data.profilePicUrl || '';
  rawMarkdownEnabled = !!data.rawMarkdownEnabled;
...
chrome.storage.onChanged.addListener((changes, area) => {
  ...
  if (changes.bottomCopyEnabled) {
    enabledState = !!changes.bottomCopyEnabled.newValue;
    if (enabledState) enableBottomCopyButtons();
    else disableBottomCopyButtons();
  }

  if (changes.oldThemeEnabled) {
    oldThemeEnabled = !!changes.oldThemeEnabled.newValue;
    if (oldThemeEnabled) applyLMArenaTheme();
    else removeLMArenaTheme();
  }

  if (changes.autoScrollDisabled) {
    autoScrollDisabled = !!changes.autoScrollDisabled.newValue;
    if (autoScrollDisabled) disableAutoScroll();
    else enableAutoScroll();
  }

  if (changes.profilePicEnabled) {
    profilePicEnabled = !!changes.profilePicEnabled.newValue;
    if (profilePicEnabled) enableProfilePicFix();
    else disableProfilePicFix();
  }

  if (changes.profilePicUrl) {
    profilePicUrl = changes.profilePicUrl.newValue || '';
    if (profilePicEnabled) {
      disableProfilePicFix();
      enableProfilePicFix();
    }
  }
  if (changes.rawMarkdownEnabled) {
    rawMarkdownEnabled = !!changes.rawMarkdownEnabled.newValue;
    if (rawMarkdownEnabled) enableRawMarkdown();
    else disableRawMarkdown();
  }
});

chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message?.type === 'GET_LATEST_USER_MESSAGE') {
    sendResponse({ text: getBottomMostUserMessageText() });
    return true;
  }

  if (message?.type === 'COPY_LATEST_USER_MESSAGE') {
    const success = copyBottomMostUserMessage();
    sendResponse({ success });
    return true;
  }

  if (message?.type === 'REFRESH_COPY_BUTTONS') {
    if (enabledState) enableBottomCopyButtons();
    else disableBottomCopyButtons();
  ...
  if (message?.type === 'REFRESH_OLD_THEME') {
    if (oldThemeEnabled) applyLMArenaTheme();
    else removeLMArenaTheme();
    sendResponse({ ok: true });
    return true;
  ...
  if (message?.type === 'REFRESH_AUTO_SCROLL') {
    if (autoScrollDisabled) disableAutoScroll();
    else enableAutoScroll();
    sendResponse({ ok: true });
    return true;
  ...
  if (message?.type === 'REFRESH_PROFILE_PIC') {
    chrome.storage.local.get(["profilePicEnabled", "profilePicUrl"], data => {
      profilePicEnabled = !!data.profilePicEnabled;
      profilePicUrl = data.profilePicUrl || '';
      if (profilePicEnabled) {
        disableProfilePicFix();
  ...
  if (message?.type === 'REFRESH_RAW_MARKDOWN') {
    if (rawMarkdownEnabled) enableRawMarkdown();
    else disableRawMarkdown();
    sendResponse({ ok: true });
    return true;
